...
<form action="{{ route('categories.update',$data->id) }}" method="post" enctype="multipart/form-data">
    ...
    <div class="w-50 mx-auto border bg-light rounded h-100 p-4 mt-5">
        <div class="mb-3 mt-3" style="text-align:center;">
            ...
            <img src={{ asset($data->image) }} style="width: 120px;max-height:120px;border-radius:100%" id="show-image" alt="">
        </div>
        <input type="text" name="current_image" value="{{ $data->image }}" hidden>
        <div class="mb-3 mt-3" >
            <label for="email" class="form-label">Add new image:</label>
            <input type="file" class="form-control"  accept="image/*" name="new_image" id="image-input" placeholder="Enter title" value="{{old('new_image')}}">
            @error('new_image')
        ...
        <div class="mb-3 mt-3">
            ...
            <input type="text" class="form-control" id="email" placeholder="Enter title" value="{{old('name') ?? $data->name }}" name="name">
            @error('name')
            <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
