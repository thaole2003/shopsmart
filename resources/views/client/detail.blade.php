          ...
          <div class="col-xxl-12">
             <div class="breadcrumb__content p-relative z-index-1">
                <div class="breadcrumb__list has-icon">
                   ...
                   <span><a href="#">@foreach ($product->categories as $index => $category)
                    {{ $category->name }}
                    @if (!$loop->last)
                        &
                    @endif
             ...
             <div class="col-xl-7 col-lg-6">
                <div class="tp-product-details-thumb-wrapper tp-tab d-sm-flex">
                        <nav>
                            <div class="nav nav-tabs flex-sm-column" id="productDetailsNavThumb" role="tablist">
                                @foreach ($product->images as $key => $image)
                                    <button class="nav-link{{ $key === 0 ? ' active' : '' }}" data-bs-toggle="tab" data-bs-target="#nav-{{ $key + 1 }}" type="button">
                        ...
                        <div class="tab-content m-img" id="productDetailsNavContent">
                            @foreach ($product->images as $key => $image)
                                <div class="tab-pane fade{{ $key === 0 ? ' show active' : '' }}" id="nav-{{ $key + 1 }}">
                                    <div class="tp-product-details-nav-main-thumb">
                                        <img style=" width: 600px;   height: 600px;object-fit: cover;" src="{{ asset($image->url) }}" alt="">
                                    </div>
                                </div>
                            @endforeach
                        </div>
             ...
             <div class="col-xl-5 col-lg-6">
                <div class="tp-product-details-wrapper">
                   <div class="tp-product-details-category">
                      <span> @foreach ($product->categories as $index => $category)
                        {{ $category->name }}
                        @if (!$loop->last)
                            &
                        @endif
                   ...
                   <h3 class="tp-product-details-title">{{ $product->nmae }}</h3>
                   ...
                   <div class="tp-product-details-price-wrapper mb-20">
                      ...
                      <input type="hidden" name="price" value="{{ ($product->price) - (($product->sale->discount/100) * ($product->price)) }}">
                      <span class="tp-product-details-price new-price">{{ ($product->price)-(($product->sale->discount/100)*($product->price))}} $</span>
                   </div>
                   ...
                   <div class="tp-product-details-variation">
                      ...
                      <div class="tp-product-details-variation-item">
                         ...
                         <div class="tp-product-details-variation-list">
                            ...
                            <label class="color tp-color-variation-btn" style="background-color: {{ $detail->color->name }};width:30px;height:30px;">
                                <input type="radio" name="detailId" value="{{ $detail->id }}" style="display: none;">
                            </label>
                            @endforeach
                         </div>
                      </div>
                   ...
                   <div class="tp-product-details-query">
                      ...
                      <div class="tp-product-details-query-item d-flex align-items-center">
                         <span>Category:  </span>
                         <p>@foreach ($product->categories as $index => $category)
                            {{ $category->name }}
                            @if (!$loop->last)
                                &
                            @endif
